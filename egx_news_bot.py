...
SIGNIFICANT_KEYWORDS = [
    ...
    "توزيعات",
    "زياده راس المال", "زيادة راس المال", "تخفيض رأس المال",
    "استحواذ", "اندماج",
    "اكتتاب", "الاكتتاب",
    ...
    with sync_playwright() as p:
        ...
        for art in collected:
            ...
            if last_error is not None:
                # Don't leak the raw exception text into the Telegram message -
                # just log it for debugging and send the item without a summary.
                print(f"DEBUG: giving up on details for {art['link']}: {last_error}")
                art["summary"] = ""
                art["pdf_links"] = []
                art["detail_failed"] = True
...
def build_message(articles, now: datetime) -> str:
    ...
    lines = [
        "📊 <b>تحديث أخبار البورصة المصرية</b>",
        f"🕒 {time_str}",
        DIVIDER,
        "",
    ]
    ...
    if not articles:
        lines.append("لا توجد أخبار جديدة في هذا التحديث.")
        return "\n".join(lines)

    significant_count = sum(
        1 for a in articles if is_significant(a["title"] + " " + a.get("summary", ""))
    )

    # --- Card per article (no ASCII table - renders badly with Arabic/RTL) ---
    for i, a in enumerate(articles, 1):
        combined_text = a["title"] + " " + a.get("summary", "")
        is_sig = is_significant(combined_text)
        ticker = extract_ticker(a["title"])
        tag = "🔥 " if is_sig else "🔹 "

        lines.append(f"{tag}<b>{i}. {ticker}</b>")
        lines.append(f"<b>{a['title']}</b>")

        # Only show a summary if the article page actually loaded - never
        # show the failure/error text itself in the Telegram message.
        if a.get("summary") and not a.get("detail_failed"):
            lines.append(a["summary"])
        elif a.get("detail_failed"):
            lines.append("ℹ️ التفاصيل غير متاحة حالياً — اضغط الرابط لعرض الخبر كاملاً.")

        for pdf in a.get("pdf_links", []):
            lines.append(f'📎 <a href="{pdf["url"]}">{pdf["label"]}</a>')

        lines.append(f'🔗 <a href="{a["link"]}">عرض الخبر على موقع البورصة</a>')
        lines.append(DIVIDER)
        lines.append("")

    lines.append(f"✅ إجمالي الأخبار الجديدة: {len(articles)}")
    if significant_count:
        lines.append(
            f"🔥 منها {significant_count} خبر هام (أرباح/خسائر/توزيعات/رأس مال/استحواذ) "
...
def build_end_of_day_summary(significant_articles, now: datetime) -> str:
    ...
    for i, a in enumerate(significant_articles, 1):
        lines.append(f"🔥 <b>{i}. {a.get('ticker', '-')}</b>")
        lines.append(f"<b>{a['title']}</b>")
        lines.append(f'🔗 <a href="{a["link"]}">عرض الخبر على موقع البورصة</a>')
        lines.append(DIVIDER)
        lines.append("")

    lines.append(f"✅ إجمالي الأخبار الهامة اليوم: {len(significant_articles)}")
...
def main():
    ...
    try:
        ...
        # Log-only: never forward raw scraping/connection errors to Telegram.
